...
# Core helper logic
def helper(df,proc):
    total_target_meters =df['monthly_mi_plan'].sum()-649037-51109
    cutoff_date = datetime(2025, 6, 30)  # CHANGE THIS VALUE as needed

    # Clean and format data
    df['Month'] = pd.to_datetime(df['Month'], format='%b %y', errors='coerce')
    df = df.dropna(subset=['Month'])
    df['total_others'] = df['total_others'].fillna(0)
    df['monthly_mi_plan'] = pd.to_numeric(df['monthly_mi_plan'], errors='coerce').fillna(0)
    df['team_avail_others'] = pd.to_numeric(df['team_avail_others'], errors='coerce').fillna(0).astype(int)
    df['installed_3ph_lt_ct'] = pd.to_numeric(df['installed_3ph_lt_ct'], errors='coerce').fillna(0)
    df['installed_ht_consumer'] = pd.to_numeric(df['installed_ht_consumer'], errors='coerce').fillna(0)
    df['installed_dt_meter'] = pd.to_numeric(df['installed_dt_meter'], errors='coerce').fillna(0)
    df['installed_feeder_meter'] = pd.to_numeric(df['installed_feeder_meter'], errors='coerce').fillna(0)
    ...
    def analyze_meter_installation(df, total_target, cutoff_date, future_months):
        ...
        # Process historical data
        historical_df = df[df['Month'] <= cutoff_date]
        total_installed = historical_df['total_others'].sum()
        remaining_meters = total_target - total_installed

        for index, row in historical_df.iterrows():

            installed_3ph_lt_ct = pd.to_numeric(row.get('installed_3ph_lt_ct', 0), errors='coerce')
            installed_ht_consumer = pd.to_numeric(row.get('installed_ht_consumer', 0), errors='coerce')
            installed_dt_meter = pd.to_numeric(row.get('installed_dt_meter', 0), errors='coerce')
            installed_feeder_meter = pd.to_numeric(row.get('installed_feeder_meter', 0), errors='coerce')
            actual_done = pd.to_numeric(row.get('total_others', 0), errors='coerce')
            team_available = pd.to_numeric(row.get('team_avail_others', 0), errors='coerce')
            

            installed_3ph_lt_ct = 0 if pd.isna(installed_3ph_lt_ct) else installed_3ph_lt_ct
            installed_ht_consumer = 0 if pd.isna(installed_ht_consumer) else installed_ht_consumer
            installed_dt_meter = 0 if pd.isna(installed_dt_meter) else installed_dt_meter
            installed_feeder_meter = 0 if pd.isna(installed_feeder_meter) else installed_feeder_meter
            actual_done = 0 if pd.isna(actual_done) else actual_done
            team_available = 0 if pd.isna(team_available) else team_available

            total_meter = installed_3ph_lt_ct + installed_ht_consumer + un_temp+installed_feeder_meter+installed_dt_meter
            total_capacity = int(team_available * 25 * proc)
            uninstalled = total_meter - actual_done
            ...
            if uninstalled > 0 and total_capacity > total_meter:
                ...
                result.append({
                    'Month': row['Month'].strftime('%b %Y'),
                    'monthly_mi_plan': row['monthly_mi_plan'],
                    'Actual MI Done_others': actual_done,
                    'team_avail_others': team_available,
                    'available_capacity': total_capacity,
                    'planned_meter_others': total_meter,
                    'revised_plan_meters': total_meter,
                    'uninstalled': uninstalled,
                    'capacity_utilized': False,
                    'capacity_shortfall': 0
        ...
        for month in future_months:
            # Get team availability for the month from the dataset
            team_available = future_df[future_df['Month'].dt.strftime('%b %Y') == month]['team_avail_others']
            team_available = team_available.iloc[0] if not team_available.empty else 0
            monthly_capacity = int(team_available * 25 * proc) if team_available > 0 else 0
            ...
            result.append({
                'Month': month,
                'revised_monthly_mi_plan': planned_meters,
                'Actual MI Done_others': 0,
                'team_avail_others ': team_available,
                'available_capacity': monthly_capacity,
                'planned_meter_others': planned_meters,
                'revised_plan_meters': installed,
                'uninstalled': uninstalled,
                'capacity_shortfall': shortfall,
                'capacity_utilized': installed == monthly_capacity
...
# Main app logic
st.title("📡 Catch Up Plan Predictor for Others")
...
if uploaded_file:
    ...
    total_target_meters = original_df['monthly_mi_plan'].sum()-649037-51109
